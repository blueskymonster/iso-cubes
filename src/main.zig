...
pub fn init(app: *App, core: *mach.Core) !void {
    ...
    const vs_module = core.device.createShaderModuleWGSL("vert.wgsl", @embedFile("shaders/vert.wgsl"));

    const vertex_attributes = [_]gpu.VertexAttribute{
        .{ .format = .float32x4, .offset = @offsetOf(Vertex, "pos"), .shader_location = 0 },
        .{ .format = .float32x4, .offset = @offsetOf(Vertex, "uv"), .shader_location = 1 },
    };
    ...
    const fs_module = core.device.createShaderModuleWGSL("frag.wgsl", @embedFile("shaders/frag.wgsl"));
...
pub fn update(app: *App, core: *mach.Core) !void {
...

    {
        ...
        const view = zm.lookAtRh(
            zm.f32x4(0, 4, 2, 1), // eye position
            zm.f32x4(0, 0, 0, 1), // focus point
            zm.f32x4(0, 0, 1, 0), // up direction
        );
